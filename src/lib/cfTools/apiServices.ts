...
class API {
    ...
    public async getServerInfo(serverId: string): Promise<ServerInfo | null> {
        try {
            ...
            const response: AxiosResponse<{ [key: string]: ServerInfo }> = await axios.get(
                `${this.url}/v1/gameserver/${serverId}`,
                {
                    headers: {
            ...
            console.error("Error during getServerInfo:", error.response ? error.response.data : error.message);
            return null;
        }
    }
